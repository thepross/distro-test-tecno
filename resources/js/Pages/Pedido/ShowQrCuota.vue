...
<template>
  <Head title="QR Cuota" />
  <AppLayout title="Pago de cuota con QR">
        ...
        <div class="col-12 col-md-7 col-lg-5">
          <div class="text-center mb-4">
            ...
            <p class="text-muted mb-1">
              Pedido #{{ cuota.plan_pago?.pedido?.id }} – Cuota #{{ cuota.numero_cuota }}
            </p>
          ...
          <div class="card shadow border-0 rounded-3">
            <div class="card-body p-4 text-center">
              <div class="alert alert-info text-start">
                <b>Modo prueba:</b> el QR cobra {{ formatMoney(montoPrueba) }}. El monto real de la cuota, {{ formatMoney(cuota.monto) }}, se envía en el detalle/motivo del QR.
              </div>

              <div class="mb-4 d-flex justify-content-center gap-2 flex-wrap">
                <span class="badge rounded-pill bg-primary px-3 py-2">Cuota {{ cuota.numero_cuota }}</span>
                <span v-if="cuota.pagofacil_transaction_id" class="badge rounded-pill bg-info px-3 py-2">
                  ID: {{ cuota.pagofacil_transaction_id }}
                </span>
              </div>
              ...
              <div class="border-top pt-3 text-start">
                <div class="d-flex justify-content-between mb-1">
                  <span class="text-muted">Monto real cuota:</span>
                  <b>{{ formatMoney(cuota.monto) }}</b>
                </div>
